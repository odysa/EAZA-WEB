 ...
 * @Last Modified time: 2020-07-02 17:50:33
 */
        ...
        if (currentProf !== "All")
          dispatch(
            actionCreater.changeGrade(
              getCumulativeGradeByProf(currentProf, grades, value)
            )
          );
        ...
        for (let i = 0; i < grades.size; ++i) {
          if (grades.get(i).get("semester") === value) {
            if (currentProf === "All")
              ...
              dispatch(
                actionCreater.changeGrade(
                  grades.get(i).get("cumulative").set("term", value)
                )
              );
            else {
              dispatch(
                actionCreater.changeGrade(
                  getGradeByProf(currentProf, grades.get(i), value)
                )
              );
            }
            break;
